...
def test_google_drive_operations():
    ...
    if folders:
        ...
        try:
            subfolders = google_api.get_folders_list(parent_folder_id=test_folder['id'])
    ...
    if inbox_folder:
        ...
        try:
            files = google_api.get_file_list_in_folder1(parent_folder_id=inbox_folder['id'])
            ...
            if test_file:
                ...
                try:
                    ...
                    print(f"   - Already trashed: {file_info.get('trashed', False)}")
                    
                    capabilities = file_info.get('capabilities', {})
                    print(f"   - Can trash: {capabilities.get('canTrash', False)}")
                    print(f"   - Can delete: {capabilities.get('canDelete', False)}")
                ...
                if result:
                    ...
                    try:
                        ...
                        if deleted_folder.id:
                            print(f"   Created/found 'deleted' folder: {deleted_folder.id}")
                            
                            # Try to move file
                            print("   Attempting to move file to 'deleted' folder...")
